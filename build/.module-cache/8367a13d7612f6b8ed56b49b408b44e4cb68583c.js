    ...
    render: function () {
        ...
        return (
            React.DOM.div( {className:"commentForm"}, 
                ...
                errorsComponent,
                React.DOM.form( {'data-abide':true, className:"commentFormForm", onSubmit:this.handleSubmit}, 
					React.DOM.div( {className:"row"}, 
						React.DOM.div( {className:"large-12 columns"}, 
							React.DOM.div( {className:"author-field"}, 
								React.DOM.label(null, "Input Label"),
								React.DOM.input( {type:"text", placeholder:"Your name", ref:"author", required:true, pattern:"alpha"} ),
								React.DOM.small( {class:"error"}, "Author is required and must be a string.")
							)
						)
					),
					React.DOM.div( {className:"row"}, 
					...
					React.DOM.div( {className:"row"}, 
						React.DOM.div( {className:"large-12 columns"}, 
							React.DOM.a( {href:"#", className:"button", ref:"submit", onClick:this.handleSubmit}, this.props.submitText)
						)
					)
                )
            )
